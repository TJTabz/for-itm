...
def shift_by_letter(letter, letter_shift):
    ...
    letter_num = ord(letter) - ord('A')
    shifted_num = (letter_num + shift_value) % 26
    shifted_letter = chr(shifted_num + ord('A'))
    return shifted_letter
...
def vigenere_cipher(message, key):
    ...
    for i in range(n):
        ...
        if current_char == ' ':
            ...
            char_code = ord(current_char) - ord('A')
            new_char_code = (char_code + shift) % 26
            res.append(chr(new_char_code + ord('A')))
...
def scytale_decipher(message, shift):
    ...
    if len(message) % shift != 0:
        num_rows += 1    
    matrix = [[''] * shift for _ in range(num_rows)]    
    ...
    for col in range(shift):
        for row in range(num_rows):
            if col * num_rows + row < len(message):
